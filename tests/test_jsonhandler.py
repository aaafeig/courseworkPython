...
def test_add_vacancy(vacancy_tested, json_handler_fixture):
    ...
    assert len(json_handler_fixture._data) == 1
    assert json_handler_fixture._data[0]["name"] == vacancy_tested.name
    assert json_handler_fixture._data[0]["salary"]["from"] == vacancy_tested.salary
    assert json_handler_fixture._data[0]["salary"]["currency"] == vacancy_tested.currency
    assert json_handler_fixture._data[0]["snippet"]["responsibility"] == vacancy_tested.description
...
def test_edit_vacancy(vacancy_tested, json_handler_fixture):
    ...
    json_handler_fixture._data = [vacancy_tested.dict_for_json]
    ...
    updated_dict = json_handler_fixture._data[0]
    assert updated_dict["name"] == "Новая вакансия"
    assert Vacancy.my_vacancies[0].name == "Новая вакансия"
    json_handler_fixture._JsonHandler__saver.assert_called_once()
